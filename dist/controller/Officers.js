...
const Officers_1 = require("../services/Officers");
...
const createPasswordController = async (req, res) => {
    try {
        ...
        const foundOfficer = await (0, Officers_1.getOfficerByEmail)(email);
        const verification = await (0, Officers_1.verifyToken)(foundOfficer._id.toString(), token);
        if (verification) {
            const createdPassword = await (0, Officers_1.createPassword)(foundOfficer._id.toString(), password);
            return res.json({
                status: "Success",
