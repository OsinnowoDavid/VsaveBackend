...
const createNewRegionController = async (req, res) => {
    try {
        ...
        const newRegion = await (0, Admin_1.createNewRegion)(regionName, location, admin, shortCode);
        if (!newRegion) {
            return res.json({
                status: "Failed",
                message: "something went wrong, try again later",
...
const assignTeamAdminToTeamController = async (req, res) => {
    try {
        const { admin, areas } = req.body;
        const foundAdmin = await (0, Admin_1.getAdminById)(admin);
        const assignAdmin = await (0, Admin_1.assignTeamAdmin)(foundAdmin._id.toString(), areas);
        const assignAdminToSubRegion = await (0, Admin_1.assignTeamAdminToTeam)(foundAdmin, areas);
        return res.json({
            status: "Success",
...
exports.assignTeamAdminToTeamController = assignTeamAdminToTeamController;
const getAllMyTeamController = async (req, res) => {
    try {
        const user = req.user;
        const foundArea = await (0, Admin_1.getAllMyTeam)(user._id.toString());
        return res.json({
            status: "Success",
...
exports.getAllMyTeamController = getAllMyTeamController;
const getTeamByRegionController = async (req, res) => {
    try {
        const { region } = req.params;
        const foundRecord = await (0, Admin_1.getTeamByRegion)(region);
        return res.json({
            status: "Success",
            message: "foundRecord",
            data: foundRecord
...
exports.getTeamByRegionController = getTeamByRegionController;
...
const getAllAgentsController = async (req, res) => {
    try {
        const foundRecords = await (0, Admin_1.getAllAgents)();
        return res.json({
            status: "Success",
            message: "found record",
            data: foundRecords
...
exports.getAllAgentsController = getAllAgentsController;
